...
export async function insertToSupabase(
  ...
  const itemRows = products.map((p, idx) => {
    ...
    return {
      ...
      source_item_no: p.source_item_no ?? p.item_code ?? null,
      delivery_no: p.delivery_no ?? null,
      customer_item_ref: p.customer_item_ref ?? null,
      unit: p.unit ?? null,
      product_name_local: p.product_name_local ?? null,
      material: p.material ?? null,
      // DB column is NOT NULL — explicit null from JS bypasses Postgres default
      source_cells: p.source_cells ?? {},
