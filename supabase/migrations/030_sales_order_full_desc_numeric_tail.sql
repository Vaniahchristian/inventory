-- Continuation / narrow tables: metrics may appear only after product text with no warehouse
-- prefix, or the first-pass trigger only scanned digits before the first 仓 token.
-- Second pass: take the last 12 numbers from the entire description (same validation as 029).
...
begin
  ...
  if v_desc <> '' and (
    ...
    -- Second pass: last 12 numbers from full description (warehouse token optional / anywhere).
